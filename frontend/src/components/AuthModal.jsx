...
export default function AuthModal({ onClose, onSuccess }) {
  ...
  const handleGoogleSignup = async () => {
    if (!googleUserData || !googleUserData.userInfo) {
    ...
    try {
      ...
      const payload = btoa(JSON.stringify({
        iss: "https://accounts.google.com",
        aud: GOOGLE_CLIENT_ID,
        sub: googleUserData.userInfo.id,
        email: googleUserData.userInfo.email,
        name: googleUserData.userInfo.name,
        picture: googleUserData.userInfo.picture,
        email_verified: googleUserData.userInfo.verified_email
      }));
      const signature = "mock_signature";
      const mockJWT = `${header}.${payload}.${signature}`;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
      ...
      {showGoogleRoleSelection && googleUserData && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4" data-testid="google-role-selection-modal" style={{pointerEvents: 'auto'}}>
